...
当前 M5.1 实现范围只覆盖 `PROJECT` 实体的服务端纵向切片：协议契约、`POST /api/v1/sync/push`、`GET /api/v1/sync/pull`、PostgreSQL 同步操作结果和项目变更日志。桌面端自动同步循环、退避重试调度、冲突处理界面、文件/合同/回款等其他实体同步均为后续工作。
