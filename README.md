...
> 当前状态：M2 账号与权限闭环、M3 项目核心在线纵向切片已完成；M4 桌面离线基础开发已完成，Windows CI 需以 GitHub Actions 实跑结果正式关闭。M5 PROJECT 同步协议、服务端事务、桌面启动/手动同步、退避重试、冲突标记和权限失效隔离已完成。文件/合同/回款等其他实体同步、生产认证仓储、签名安装包和生产部署仍放在后续里程碑。
